          ...
          <div className="flex items-start justify-center gap-3">
            {['DAYS', 'HOURS', 'MINUTES', 'SECONDS'].map((label, i) => (
              <div key={label} className="flex flex-col items-center gap-2">
                {i > 0 && (
                  <span style={{ color: COLORS.gold, fontSize: '1.5rem', marginTop: '20px', fontFamily: 'Cormorant Garamond', position: 'absolute', marginLeft: -28 }}>:</span>
                )}
                <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, width: 72, height: 80 }} />
                <span style={{ fontSize: '0.65rem', color: COLORS.mid, letterSpacing: '0.15em', fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic' }}>{label}</span>
              </div>
            ))}
          </div>
        ) : time.past ? (
          <div className="text-center" style={{ color: COLORS.rose, fontSize: '1.2rem' }}>
            결혼식 날이 됐습니다! 축하합니다!
          </div>
        ) : (
          <>
            ...
            >
              저희 결혼식까지{' '}
              <strong style={{ color: COLORS.rose }}>{time.days}일</strong> 남았습니다
            </p>
          </>
        )}
      </div>
